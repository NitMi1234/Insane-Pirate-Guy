...
function preload() {
  ...
  waterSound = loadSound("assets/Splah.mp3")
  ...
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      ...
      if (collision.collided && !boats[i].isBroken) {
       //if pirate isnt laughing and sound isnt playing then play the sound and make it laugh
        if(!pirateLaugh.isPlaying()&&!isLaughing){
            pirateLaugh.play();
            isLaughing=true 
        }
        //if the boat touches the tower we lose the game and game over function will be called
        isGameOver = true;
        gameOver();
      }
    }
...
function keyReleased() {
  if (keyCode === DOWN_ARROW && !isGameOver) {
    ...
    cannonExplosionSound.setVolume(0.03);
  }
}
